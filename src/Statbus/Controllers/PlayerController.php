...
class PlayerController Extends Controller {
  ...
  public function getPlayerByCkey($ckey, $public = false){
    if($public){
      return $this->DB->row("SELECT player.ckey
        FROM player
        WHERE player.ckey = ?", $ckey);
    }
    return $this->DB->row("SELECT player.ckey,
      player.firstseen,
      player.firstseen_round_id,
      player.lastseen,
      player.lastseen_round_id,
      player.ip,
      player.computerid,
      admin.rank,
      player.accountjoindate,
      player.flags,
      count(DISTINCT connection_log.id) AS connections,
      G.minutes as ghost,
      L.minutes as living,
      floor((G.minutes + L.minutes) / 60) AS hours,
      DATEDIFF(CURDATE(),player.lastseen) AS days
      FROM player
      LEFT JOIN role_time AS G ON G.ckey = player.ckey AND G.job = 'Ghost'
      LEFT JOIN role_time AS L ON L.ckey = player.ckey AND L.job = 'Living'
      LEFT JOIN admin ON player.ckey = admin.ckey
      LEFT JOIN connection_log ON connection_log.ckey = player.ckey
      WHERE player.ckey = ?", $ckey);
  }
  ...
  public function getPlayerByIP(int $IP){
   ...
   public function getRoleData($ckey, $start = null, $end = null, $jobs) {
    return json_encode($this->DB->run("SELECT job, SUM(delta) AS minutes
      FROM role_time_log
      WHERE ckey = ?
      AND role_time_log.job IN $jobs
      AND role_time_log.datetime BETWEEN ? AND ?
      GROUP BY job
  ...
  public function findAlts($ckey){
    $alts = $this->DB->run("SELECT
      ...
      C.ckey AS cid_alts
      FROM player AS P
      LEFT JOIN connection_log AS I ON I.ip = P.ip AND I.ckey != P.ckey
      LEFT JOIN connection_log AS C ON C.computerid = P.computerid AND P.ckey != C.ckey
      WHERE P.ckey = ?
  ...
  public function countRounds($ckey){
    return $this->DB->cell("SELECT count(round.id) FROM connection_log
      LEFT JOIN round ON connection_log.round_id = round.id
      WHERE connection_log.ckey = ?
      AND round.shutdown_datetime IS NOT NULL", $ckey);
  }

  public function countMessages($ckey){
    return $this->DB->cell("SELECT count(M.id) FROM messages M
      WHERE M.deleted = 0
  ...
  public function getLastWords($ckey){
    return $this->DB->run("SELECT last_words, id FROM death WHERE byondkey = ? AND last_words IS NOT NULL AND last_words != '';", $ckey);
  }
  ...
  public function findCkeys($request, $response, $args){
    ...
    $ckey = filter_var($args['ckey'], FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH);
    if ($ckey){
      $results = $this->DB->run("SELECT player.ckey FROM player
        WHERE player.ckey LIKE ?
